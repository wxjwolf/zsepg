    ...
    with requests.request('GET',url1,headers = {'User-agent':ua}) as res:
        ...
        if load_data.get("message")=="success":
            ...
            for i in data:
                ...
                ch.append({'channel':pid,'start':formattime(i["play_time"]),'stop':datetime.fromtimestamp(endtime/1000.0).strftime("%Y%m%d%H%M%S"),'title':i["program_title"]})
    return ch;

def formattime(t):
       return datetime.fromtimestamp(float(t)/1000.0).strftime("%Y%m%d%H%M%S")    
